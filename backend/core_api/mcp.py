...
...) can call. Nothing per resource to write: a resource gets tools as
soon as it's registered the usual way (a `BaseViewSet` subclass plus a
`register_model_endpoint` call in its app's urls.py). Set
`mcp_enabled = False` on a viewset to leave it out.

Every tool call is an internal sub-request to the resource's REAL API URL
(resolved through the host's own urlconf, dispatched to the viewset),
carrying the MCP request's own headers - so it's authenticated,
...
messages; `GET` isn't offered (405). Mount it wherever the host likes:

    path("api/v1/mcp", McpView.as_view(permission_classes=[IsAuthenticated]))

`permission_classes` there only gates the MCP endpoint itself (so an
unauthenticated client gets a plain 401 up front); each tool's own call
still goes through its viewset's permissions regardless.
"""
...
def _call_api(request, method: str, path: str, *, query=None, body=None) -> tuple[int, object]:
    ...
    caller had sent it - same headers (auth, cookies), same remote
    address. Returns `(status, parsed JSON body or None)`."""
...
class McpView(APIView):
    """The MCP endpoint - see this module's docstring. Settings read (all
