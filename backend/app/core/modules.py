...
Reads `modules.yaml` at the platform-core repo root. See
docs/architecture/microservices-design.md (in the GoalNexa repo) for the
planned dynamic version (`POST /internal/modules/register` at runtime,
storing the manifest and marking modules `discovered`/`enabled`); this
static file is today's stand-in, before any module actually registers
...
_MODULES_FILE = Path(__file__).resolve().parents[3] / "modules.yaml"
