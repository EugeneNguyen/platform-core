...
# Fixed bigint key for `pg_advisory_xact_lock`, acquired by every
# `POST /auth/signup` call before its bootstrap-closed exists-check.
...
# beyond "a fixed constant every signup call agrees on") — derived once, at
# write-time, from a fixed string through Python's `zlib.crc32`, purely so
# it's stable and doesn't collide with a small integer app code might
# otherwise pick out of habit.
