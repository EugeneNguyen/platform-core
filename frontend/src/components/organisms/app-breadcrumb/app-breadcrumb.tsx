/**
 * route-derived breadcrumb, mounted once inside
 * `AppShell` alongside `AppFooter` so both join `AppSidebar`/`AppHeader` in
 * completing the free-template shell shape.
 *
 ...
 * the literal string "pick" — retired 2026-09-13, /, along
 * with the route itself.)
 *
 * Graceful degradation: any path with no matching table
 ...
 * (`ProjectDetail`/`BatchDetail`/`RoundDetail`, generic admin
 ...
 * deliberately does not do), so the Project/Batch/Round chain
 ...
 * (`OrgHome`, `OrgMembers`, `ProjectDetail`, `BatchDetail`,
 * `RoundDetail`) is fixed to match in the same pass — see each file's
 ...
 *
 * Raw HTML (2026-09-07), not `@coreui/react` — this is the
 ...
 *
 * AdminLTE v4: the `ROUTE_BREADCRUMBS` table, the `matchPath`
 ...
 *
 *: this component is no longer render-only
 ...
 * click-path back to the org's Projects list ( `/orgs/:orgId/projects`,
 * — no new route or affordance invented for this).
 *
 ...
 * same commit ( own Consequences names this; `AppBreadcrumb.test.tsx`
 ...
 * — note it leaves `AppShell`'s `.app-content-header` div
...
/**
 *: resolved nav context passed alongside the route's own
 ...
 * same invariant already pins for unmapped routes, now also
 * covering this new failure source.
 *
 ...
 * partial-label branch exists to forbid.
 *
 ...
 * which is exactly the partial state §4 rules out.
 */
...
const ROUTE_BREADCRUMBS: RouteBreadcrumbConfig[] = [
  ...
  {
    //: the retired bespoke `ProjectsPage` had no dedicated edit
  ...
  {
    //: must precede the generic `/orgs/:orgId` catch-all
  ...
  {
    //: the read-only detail view. `matchPath` is exact by default,
  ...
  //: all 5 project-scoped patterns below open with the
  ...
  {
    pattern: "/projects/:projectId/batchs/:testPlanId/rounds/:testCycleId",
    segments: (params, context) =>
      projectTrail(context, `/projects/${params.projectId}`, [
        {
          label: "Test Plan",
          to: `/projects/${params.projectId}/batchs/${params.testPlanId}`,
        },
        { label: "Test Cycle" },
      ]),
  },
  {
    pattern: "/projects/:projectId/batchs/:testPlanId",
    segments: (params, context) =>
  ...
  {
    //: the project-scoped half of the same read-only detail view.
...
function AppBreadcrumb() {
  ...
  //: called unconditionally at the top of the body, before
