/**
 * app header, mounted once inside `AppShell` (via `ProtectedRoute`)
 ...
 *.
 *
 * adds the sidebar toggler button: calls the
 ...
 * `AuthContext.tsx`/) and then navigates to `/login` via React
 ...
 *
 * adds the dark/light color-mode
 ...
 *
 * adds the organization-switcher
 ...
 *
 * 1. **Lazy, uncached fetch**. Fires on open — never on
 ...
 * page reload. A header control
 * present on every protected screen must survive a reload, so it owns
 * its own fetch. accepts the extra request per open as the
 * cost of not having a cache-invalidation story yet.
 * 2. **Always rendered**, even at exactly one org — no
 ...
 * 3. **Empty and error states are distinct**: "No
 ...
 * an attempt to carry the current nested sub-route across: a
 ...
 *
 * Raw HTML (2026-09-07), not `@coreui/react` — this file
 ...
 *: managing a vanilla-JS component
 ...
 * preserving "no caching across opens" behaviour.
 ...
 * `data-bs-toggle` attribute is set, so Tabler's own loaded JS
 * never sees this button, avoiding the duplicate-listener risk
 ...
 * deliberate deviation, Decision for why.
 * - **Color mode storage key renames again**: the attribute
 ...
 * §4.3 already accepted the first time this key was renamed. The
...
/** Tabler's own `tabler-theme.js` storage-key convention. */
 ...
 * from "fetched and empty" is what keeps two states honest.
 */
type OrgListState =
  | { status: "idle" }
  | { status: "loading" }
...
function AppHeader({ onToggleSidebar }: AppHeaderProps) {
  ...
  function handleOrgDropdownHide() {
    ...
    // previous open's list — "no caching across opens" is a
      ...
      <div className="container-fluid d-flex justify-content-between align-items-center">
        <div className="d-flex align-items-center">
          <button
            ...
            // loaded JS must never see a toggle attribute it
          ...
          {/* follow-up (2026-09-09, direct CTO instruction): the
              header carries no brand mark at all — the sidebar (always
              visible, ) is the single source of the brand mark
              now. Decision §7 originally put a small mark here;
