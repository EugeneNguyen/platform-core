 ...
 *
 * ( originally scoped this surface to exactly two page components,
 * `EntityListPage` + `EntityFormPage`. extends that to three — see
 ...
 * entities today (`Batch`'s `scope`/`approach`/`staffing_and_training`/
 * `schedule`, `Item`'s `preconditions`/`expected_result`, `Issue`'s
 * `reported_by_actor_id`, `Run`'s `actual_result`/
 * `executed_by_actor_id`, `RiskNote`'s `mitigation`, plus every entity's
 ...
 * `useEntitySchema`, ) plus one `GET {path}/{id}` (`getEntity`). There
 ...
 * renders no tab strip at all and is byte-for-byte the pre- page.
 *
 ...
 * fetch its own row rather than reuse the list's in-memory copy. An
 ...
 * "use the library's own documented class names verbatim" rule.
 */
...
/**
 *: a relationship's tab id. Keyed on the **listed** entity
 * (`relation.entity`), not on `targetEntity` or the label: `Spec`
 * reaches `Criterion` both directly and through a traceability link, so
...
/**
 *: `entityKeyOverride` mirrors `EntityListPage`/`EntityFormPage`'s own
 * prop, for a future route with no `:entity` segment. No such mount
 * exists today — `Project`'s own detail view is the bespoke `ProjectDetail`
 * workspace, reached via `EntityConfig.detailPath`, which this
...
function EntityDetailPage({ entityKeyOverride }: { entityKeyOverride?: string } = {}) {
  ...
  /**
   *: paging state for whichever relationship tab is open — one pair
  ...
  /**
   *. `?tab=` is validated against the served relationship set rather
  ...
  /**
   * (Amendment): above the card, not inside its header — Tabler's
          ...
          >
            {activeRelation ? (
              <EntityRelationTab
                ...
                /**
                 *: the tab's write actions are permission-gated, and
