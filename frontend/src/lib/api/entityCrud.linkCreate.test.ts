/**
 * — `interpolateLinkPath` / `createLinkRow`.
 *
...
const ACTION = {
  pathTemplate: "/specs/{spec_id}/item-links/{item_id}",
  permission: "spec_item_link.create",
};
...
describe("interpolateLinkPath ", () => {
  it(": fills every {field} placeholder from the matching key", () => {
    expect(
      interpolateLinkPath(ACTION.pathTemplate, {
        spec_id: REQUIREMENT_ID,
        item_id: CASE_ID,
      }),
    ).toBe(`/specs/${REQUIREMENT_ID}/item-links/${CASE_ID}`);
  });

  it(": substitutes by NAME, not by position", () => {
    /**
     ...
     */
    const swappedKeyOrder = { item_id: CASE_ID, spec_id: REQUIREMENT_ID };
    expect(interpolateLinkPath(ACTION.pathTemplate, swappedKeyOrder)).toBe(
      `/specs/${REQUIREMENT_ID}/item-links/${CASE_ID}`,
    );
  });

  it(": throws rather than POSTing a URL with a literal brace in it", () => {
    /**
     * A missing value is a programming error (the tab always holds both ids),
     * and the alternative — leaving `{item_id}` in the path — produces a
     ...
     */
    expect(() => interpolateLinkPath(ACTION.pathTemplate, { spec_id: REQUIREMENT_ID })).toThrow(
      /item_id/,
    );
    expect(() =>
      interpolateLinkPath(ACTION.pathTemplate, { spec_id: REQUIREMENT_ID, item_id: "" }),
    ).toThrow(/item_id/);
  });
...
describe("createLinkRow ", () => {
  ...
  it(": POSTs the interpolated path under /api/v1 with no request body", async () => {
    const fetchMock = vi.fn(async () => jsonResponse({ spec_id: REQUIREMENT_ID, item_id: CASE_ID }));
    vi.stubGlobal("fetch", fetchMock);

    await createLinkRow(ACTION, { spec_id: REQUIREMENT_ID, item_id: CASE_ID });

    const [url, init] = fetchMock.mock.calls[0] as unknown as [string, RequestInit];
    // The prefix is added here, never in the template — a template carrying
    // its own would produce `/api/v1/api/v1/...`.
    expect(url).toBe(`/api/v1/specs/${REQUIREMENT_ID}/item-links/${CASE_ID}`);
    expect(init.method).toBe("POST");
    // Both ids travel in the path; the route parses no body at all.
    expect(init.body).toBeUndefined();
  });

  it(": surfaces the route's own error envelope as an ApiError", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
        jsonResponse(
          { code: "link_already_exists", message: "This test case is already linked to this spec.", field_errors: null },
          409,
        ),
      ),
    );
    ...
    await expect(
      createLinkRow(ACTION, { spec_id: REQUIREMENT_ID, item_id: CASE_ID }),
    ).rejects.toMatchObject({ message: "This test case is already linked to this spec." });
  });
});
