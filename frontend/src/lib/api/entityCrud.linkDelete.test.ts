...
const ACTION = {
  ...
  permission: "spec_item_link.delete",
};
...
describe("deleteLinkRow ", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    clearAccessToken();
  });

  it(": DELETEs the interpolated path under /api/v1 with no request body", async () => {
    const fetchMock = vi.fn(async () => noContentResponse());
    vi.stubGlobal("fetch", fetchMock);

    await deleteLinkRow(ACTION, { spec_id: REQUIREMENT_ID, item_id: CASE_ID });
  ...
  it(": resolves undefined on 204 rather than trying to parse an empty body", async () => {
    /**
    ...
    await expect(
      deleteLinkRow(ACTION, { spec_id: REQUIREMENT_ID, item_id: CASE_ID }),
    ).resolves.toBeUndefined();
  });

  it(": substitutes by NAME, so either end of the junction builds the same URL", () => {
    /**
     ...
     * makes — the key order it hands in follows that
     * tab's own `scopeField`/`targetField`, not the path's segment order.
     */
    const fetchMock = vi.fn(async () => noContentResponse());
    vi.stubGlobal("fetch", fetchMock);

    return deleteLinkRow(ACTION, { item_id: CASE_ID, spec_id: REQUIREMENT_ID }).then(() => {
      const [url] = fetchMock.mock.calls[0] as unknown as [string, RequestInit];
      expect(url).toBe(`/api/v1/specs/${REQUIREMENT_ID}/item-links/${CASE_ID}`);
    });
  });

  it(": surfaces a 404 for an already-removed pair as an ApiError with the route's own message", async () => {
    /**
    ...
    vi.stubGlobal(
      ...
      vi.fn(async () =>
        jsonResponse(
          {
            code: "not_found",
            message: "This test case is not linked to this spec.",
            field_errors: null,
          },
          404,
        ),
    ...
    await expect(
      deleteLinkRow(ACTION, { spec_id: REQUIREMENT_ID, item_id: CASE_ID }),
    ).rejects.toMatchObject({ message: "This test case is not linked to this spec." });
  });

  it(": surfaces a 403 distinctly from the 404 above", async () => {
    /**
